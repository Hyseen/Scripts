...
;(async () => {
  ...
  let subPolicies = await getSubPolicies(disneyPolicyName).catch(error => {
    console.log(error)
  ...
  let avaliablePolicies = await testPolicies(subPolicies)
  $.setval(JSON.stringify(avaliablePolicies), 'Helge_0x00.Disney_Avaliable_Policies')
})()
  .catch(error => {
    console.log(error)
  })
...
async function testPolicies(policies = []) {
  ...
  if (retry && faildPolicies.length > 0) {
    await Promise.map(faildPolicies, subPolicy => test(subPolicy), { concurrency: 5 })
      .then(echo)
      .catch(error => console.log(error))
  }
