...
;(async () => {
  ...
  let { selected, actual } = await getSelectedPolicy(disneyPolicyName)
  if (!selected) {
    $.msg($.name, '', '⚠️ Disney+ 策略组名未填写或填写有误')
    return
  }

  if (debug) {
    console.log(`${disneyPolicyName} 策略组当前选择的策略：${selected}, 使用节点：${actual}`)
  }
  ...
  if (status === STATUS_AVAILABLE) {
    ...
    $.msg($.name, `${actual}`, `该节点支持 Disney+ ➟ ${flag} ${regionName}`)
    return
  }

  let cachePolicies = []
  try {
    cachePolicies = JSON.parse($.getval('Helge_0x00.Disney_Avaliable_Policies') ?? '[]')
  } catch (error) {
  ...
  let subPolicies = await getSubPolicies(disneyPolicyName)
  cachePolicies = cachePolicies.filter(item => subPolicies.indexOf(item.policy) !== -1 && item.policy !== selected)
  ...
  $.setval(JSON.stringify(cachePolicies), 'Helge_0x00.Disney_Avaliable_Policies')
  if (cachePolicies.length <= 0) {
    $.msg($.name, '', '⚠️ 没有可用策略，请先运行 「Disney+ 解锁检测」脚本')
    return
  }

  let { policy, region: newRegion } = cachePolicies[0]
  let { success, actual: newActual } = await setPolicyState(disneyPolicyName, policy)

  if (success) {
    let flag = getCountryFlagEmoji(newRegion) ?? ''
    let regionName = RESION_NAMES?.[newRegion.toUpperCase()]?.chinese ?? ''
    $.msg($.name, `切换 ${actual} ➟ ${newActual}`, `该节点支持 Disney+ ➟ ${flag} ${regionName}`)
  } else {
    $.msg($.name, '', `切换失败，请重试`)
  }
})()
...
async function test(policyName) {
  try {
    let { countryCode, inSupportedLocation } = await Promise.race([getLocationInfo(policyName), timeout(8000)])
    if (debug) {
    ...
    if (inSupportedLocation === true || inSupportedLocation === 'true') {
      return { region: countryCode, status: STATUS_AVAILABLE, policy: policyName }
    }

    let { status } = await Promise.race([getHomePage(policyName), timeout(8000)])
    if (debug) {
    ...
    if (status === STATUS_COMING) {
      return { region: countryCode, status: STATUS_COMING, policy: policyName }
    }

    // 不支持 Disney+
    return { status: STATUS_NOT_AVAILABLE, policy: policyName }
  } catch (error) {
    ...
    // 检测超时
    if (error === 'Timeout') {
      return { status: STATUS_TIMEOUT, policy: policyName }
    }

    return { status: STATUS_ERROR, policy: policyName }
  }
}

async function testPolicies(policies = []) {
  let avaliablePolicies = []
  let echo = results => {
    console.log(`策略组检测结果：`)
    for (let { policy, status, region } of results) {
      switch (status) {
        case STATUS_COMING: {
          let flag = getCountryFlagEmoji(region) ?? ''
          let regionName = RESION_NAMES?.[region.toUpperCase()]?.chinese ?? ''
          console.log(`${policy}: 即将登陆 ➟ ${flag}${regionName}`)
          break
        }
        case STATUS_AVAILABLE: {
          ...
          avaliablePolicies.push({ policy, region })
          break
        }
