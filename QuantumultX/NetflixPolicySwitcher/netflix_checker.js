...
;(async () => {
  ...
  if (sortByTime) {
    fullAvaliablePolicies = fullAvaliablePolicies.sort((m, n) => m.time - n.time)
    originalAvaliablePolicies = originalAvaliablePolicies.sort((m, n) => m.time - n.time)
  }
...
async function testPolicies(policies = []) {
  ...
  if (retry && faildPolicies.length > 0) {
    await Promise.map(faildPolicies, subPolicy => test(subPolicy), { concurrency })
      .then(echo)
      .catch(error => console.log(error))
  }
