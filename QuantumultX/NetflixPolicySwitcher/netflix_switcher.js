...
// 完整支持
const STATUS_FULL_AVALIABLE = 2
// 支持自制剧
const STATUS_ORIGINAL_AVAILABLE = 1
// 不支持解锁
...
;(async () => {
  if (!$.isQuanX()) {
    $.msg($.name, '', '⚠️ 该脚本仅支持 QuantumltX')
    return
  }
  let { selected, actual } = await getSelectedPolicy(policyName)
  if (!selected) {
    $.msg($.name, '', '⚠️ Netflix 策略组名未填写或填写有误')
    return
  }

  if (debug) {
    console.log(`${policyName} 策略组当前选择的策略：${selected}, 使用节点：${actual}`)
  }

  let { region, status } = await test(selected)
  if (status === STATUS_FULL_AVALIABLE) {
    let flag = getCountryFlagEmoji(region) ?? ''
    let regionName = RESION_NAMES?.[region.toUpperCase()]?.chinese ?? ''
    $.msg($.name, `${actual}`, `该节点完整支持 Netflix ➟ ${flag} ${regionName}`)
    return
  }

  let cacheFullPolicies = []
  try {
    cacheFullPolicies = JSON.parse($.getval('Helge_0x00.Netflix_Full_Avaliable_Policies') ?? '[]')
  } catch (error) {
  ...
  try {
    cacheOriginalPolicies = JSON.parse($.getval('Helge_0x00.Netflix_Original_Avaliable_Policies') ?? '[]')
  } catch (error) {
  ...
  let subPolicies = await getSubPolicies(policyName)
  cacheFullPolicies = cacheFullPolicies.filter(item => subPolicies.indexOf(item.policy) !== -1 && item.policy !== selected)
  cacheOriginalPolicies = cacheOriginalPolicies.filter(item => subPolicies.indexOf(item.policy) !== -1 && item.policy !== selected)

  // 切换前重新检测是否可用
  if (recheck) {
    let { fullAvaliablePolicies, originalAvaliablePolicies } = await testPolicies(cachePolicies.map(item => item.policy))
    cacheFullPolicies = fullAvaliablePolicies
    cacheOriginalPolicies = originalAvaliablePolicies
  }

  $.setval(JSON.stringify(cacheFullPolicies), 'Helge_0x00.Netflix_Full_Avaliable_Policies')
  $.setval(JSON.stringify(cacheOriginalPolicies), 'Helge_0x00.Netflix_Original_Avaliable_Policies')

  if (cacheFullPolicies.length <= 0 && cacheOriginalPolicies <= 0) {
    $.msg($.name, '', '⚠️ 没有可用策略，请先运行 「Netflix 解锁检测」脚本')
    return
  }
  ...
  let { success, actual: newActual } = await setPolicyState(policyName, newPolicy)

  if (success) {
    let flag = getCountryFlagEmoji(newRegion) ?? ''
    let regionName = RESION_NAMES?.[newRegion.toUpperCase()]?.chinese ?? ''
    $.msg(
      $.name,
      `切换 ${actual} ➟ ${newActual}`,
      newStatus === STATUS_FULL_AVALIABLE ? `该节点完整支持 Netflix ➟ ${flag} ${regionName}` : `该节点仅支持 Netflix 自制剧 ➟ ${flag} ${regionName}`
    )
  } else {
    $.msg($.name, '', `切换失败，请重试`)
  }
...
function getSelectedPolicy(policyName) {
  return new Promise((resolve, reject) => {
    let message = {
    ...
    $configuration.sendMessage(message).then(
      response => {
        if (debug) {
          console.log(`get_policy_state: ${JSON.stringify(response)}`)
        }

        if (response.error) {
          console.log(`get_policy_state error: ${response.error}`)
          resolve({})
          return
        }

        if (response.ret) {
          let policies = response.ret?.[policyName]
          if (policies.length >= 2) {
            let selected = policies[1]
            let actual = policies[policies.length - 1]
...
function getSubPolicies(policyName) {
  return new Promise((resolve, reject) => {
    let message = {
    ...
    $configuration.sendMessage(message).then(
      response => {
        if (debug) {
          console.log(`getSubPolicies: policyName=${policyName}, ${JSON.stringify(response)}`)
        }
        ...
        if (response.ret) {
          let policy = response.ret?.[policyName]
...
function setPolicyState(proxy, cPolicy) {
  return new Promise((resolve, reject) => {
    let message = {
      action: 'set_policy_state',
      content: { [proxy]: cPolicy },
    }

    $configuration.sendMessage(message).then(
      response => {
        if (debug) {
          console.log(`set_policy_state: proxy=${proxy}, cPolicy=${cPolicy}`)
        }

        if (response.error) {
          console.log(`set_policy_state error: ${response.error}`)
          resolve({ success: false })
          return
        }

        if (response.ret) {
          let policies = response.ret?.[proxy]
          if (policies.length >= 2) {
            let selected = policies[1]
            let actual = policies[policies.length - 1]
            resolve({ success: selected == cPolicy, actual })
            return
          }
        }
...
async function testPolicies(policies = []) {
  let fullAvaliablePolicies = []
  let originalAvaliablePolicies = []
  let echo = results => {
    console.log(`策略组检测结果：`)
    for (let { policy, status, region } of results) {
      switch (status) {
        case STATUS_FULL_AVALIABLE: {
          let flag = getCountryFlagEmoji(region) ?? ''
          let regionName = RESION_NAMES?.[region.toUpperCase()]?.chinese ?? ''
          console.log(`${policy}: 完整支持 Netflix ➟ ${flag}${regionName}`)
          fullAvaliablePolicies.push({ policy, region, status })
          break
        }
        case STATUS_ORIGINAL_AVAILABLE: {
          ...
          originalAvaliablePolicies.push({ policy, region, status })
          break
        }
  ...
  await Promise.map(policies, subPolicy => test(subPolicy), { concurrency: 10 })
    .then(echo)
    .catch(error => console.log(error))

  return { fullAvaliablePolicies, originalAvaliablePolicies }
}
...
async function test(policyName) {
  return await Promise.race([getFilmPage(81215567, policyName), timeout()])
    .then(region => {
      return { region, policy: policyName, status: STATUS_FULL_AVALIABLE }
    })
    .catch(async error => {
      if (error !== 'Not Found') {
