...
;(async () => {
  ...
  // 切换前重新检测是否可用
  if (recheck) {
    let { fullAvaliablePolicies } = await testPolicies(cacheFullPolicies.map(item => item.policy))
    ...
    for (let { policy, status, region, time } of results) {
      switch (status) {
        case STATUS_FULL_AVALIABLE: {
          ...
          fullAvaliablePolicies.push({ policy, region, status, time })
          break
        }
        case STATUS_ORIGINAL_AVAILABLE: {
          ...
          originalAvaliablePolicies.push({ policy, region, status, time })
          break
        }
